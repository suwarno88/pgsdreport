...
# Jika API Key tidak ada, aplikasi tidak akan crash, tapi akan memunculkan error saat fitur AI dipakai
client = OpenAI(api_key=API_KEY) if API_KEY else None
OPENAI_MODEL = "gpt-4.1" # Catatan: Pastikan nama model ini sesuai dengan akses API Anda (misal: gpt-4o, gpt-4-turbo)
...
# ── Custom CSS (Terakhir Termasuk Glassmorphism untuk Chat) ──────────────────
st.markdown("""
<style>
...
.kpi-card {
    ...
    padding: 1.4rem 1.6rem; text-align: center; box-shadow: 0 4px 20px rgba(0,0,0,0.2);
}
...
[data-testid="stChatMessage"] {
    background: rgba(25, 30, 45, 0.5) !important;
...
/* Membedakan warna chat user dan AI */
...
COLORS = {"q1": "#6366f1", "realization": "#22d3ee", "success": "#34d399", "warning": "#fbbf24", "danger": "#f87171"}

def safe_pct(real, target): return (real / target) * 100 if (pd.notna(real) and pd.notna(target) and target != 0) else None
        ...
        with st.spinner(f"Menganalisis performa {selected_quarter}..."):
            try:
                ...
                response = client.chat.completions.create(
                    model=OPENAI_MODEL,
                    messages=[
                        {"role": "system", "content": "You are a helpful academic data analyst."},
...
<div class="hero-banner">
    ...
    <p>Jurusan Pendidikan Guru Sekolah Dasar (PGSD) — Tahun 2026 &nbsp;|&nbsp; Kuartal: <strong>{selected_quarter}</strong></p>
</div>
""", unsafe_allow_html=True)
...
with tab1:
    if len(valid_comp) > 0:
        ...
        fig_bar.update_layout(
            PLOTLY_LAYOUT, barmode="group", height=max(450, len(chart_df) * 55),
...
with tab2: st.info("Tampilan analisis data (gunakan format script grafik radar/line chart sebelumnya di sini).")
with tab3: st.dataframe(filtered_df, use_container_width=True)

# ━━ TAB 4: SECURE AI CHATBOT BACKEND ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
with tab4:
    st.markdown("### 💬 Asisten Data Interaktif")
    st.caption("Tanyakan apa saja mengenai data performa di atas. Asisten ini memproses data secara aman di sisi server.")
    
    # Inisialisasi memori percakapan
    if "messages" not in st.session_state:
        st.session_state.messages = [
            {"role": "assistant", "content": "Halo! Saya AI Assistant PGSD. Data Anda aman. Apa yang ingin Anda analisis hari ini?"}
        ]

    # Render history percakapan dengan efek glassmorphism bawaan CSS di atas
    for message in st.session_state.messages:
        with st.chat_message(message["role"]):
            st.markdown(message["content"])

    # Kotak Input User
    if prompt := st.chat_input("Tanya tentang pencapaian target, indikator fokus, dll..."):
        ...
        # 3. Ekstrak data saat ini sebagai konteks (agar AI cerdas & relevan)
        chat_context_data = filtered_df[["Performance Indicator", quarter_col, "Realization", "PI Focus"]].to_dict(orient="records")
        system_prompt = f"""
        You are an AI assistant embedded in a PGSD Performance Dashboard.
        CRITICAL INSTRUCTIONS:
        1. You MUST ONLY answer questions related to the following JSON data.
        2. If the user asks anything outside of this data, politely decline.
        3. LANGUAGE MATCHING: If the user speaks Indonesian, reply in Indonesian. If English, reply in English.
        4. Use Markdown for rich text formatting (bold, bullet points, etc).
        DASHBOARD DATA:
        ...
        # 4. Panggil OpenAI dari Backend Python secara streaming
            ...
            if not client:
                ...
                try:
                    stream = client.chat.completions.create(
                        ...
                        stream=True, # Ini mengaktifkan efek Typewriter yang sangat cepat
                    )
                    # write_stream akan membaca data sepotong demi sepotong layaknya ChatGPT
